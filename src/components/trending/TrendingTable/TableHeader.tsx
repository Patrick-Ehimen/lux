...
const TableHeader: React.FC<TableHeaderProps> = ({
  ...
  return (
    <tr className="text-xs uppercase text-dex-text-secondary border-b border-dex-border">
      {/* Sticky columns */}
      <th className="sticky left-0 z-20 bg-dex-bg-primary px-4 py-3 text-left w-8">
        #
      </th>
      <th className="sticky left-8 z-20 bg-dex-bg-primary px-4 py-3 text-left w-72 border-r border-dex-border">
        TOKEN
      </th>
      {/* Scrollable columns */}
      <th
        className="px-6 py-3 text-right cursor-pointer min-w-[100px]"
        onClick={() => onSortChange("usdPrice")}
      >
        PRICE {renderSortIcon("usdPrice")}
      </th>
      <th
        className="px-6 py-3 text-right cursor-pointer min-w-[80px]"
        onClick={() => onSortChange("createdAt")}
      >
        AGE {renderSortIcon("createdAt")}
      </th>
      <th
        className="px-6 py-3 text-right cursor-pointer min-w-[90px]"
        onClick={() => onSortChange("transactions.24h")}
      >
        TXNS {renderSortIcon("transactions.24h")}
      </th>
      <th
        className="px-6 py-3 text-right cursor-pointer min-w-[110px]"
      ...
      {/* <th
        className="px-6 py-3 text-right cursor-pointer min-w-[70px]"
